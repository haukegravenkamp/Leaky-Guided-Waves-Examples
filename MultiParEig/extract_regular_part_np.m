...
%  - mingap : (1) minimum required gap (log10) between singular values at rank
%
% Output:
...
% BP 19.12.2023 : rank criteria is rankeps * norms of initial Delta matrices
...
for j = 2:length(Delta)
    tmp(j) = norm(Delta{j},'fro');
end
opts.initialnorms = [norm(Delta{1},'fro') norm(tmp)];
...
for k = 1:size(RS,1)
    ...
    report = [report; rep];
    % if the gap between last "nonzero" and first "zero" singular value is 
    % too small, the algorithm stops
